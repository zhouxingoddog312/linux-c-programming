...
### 11.10
可以定义vector<int>::iterator到int的map，因为此类型的迭代器支持<操作；不能定义list<int>::iterator到int的map，因为此类型的迭代器不支持<操作。
### 11.11
...
```
### 11.15
对一个`int`到`vector<int>`的map，其mapped_type是`vector<int>`,key_type是`int`，value_type是`pair<const int,vector<int>>`。
### 11.16
...
### 11.29
如果给定关键字不在容器中，upper_bound与lower_bound会返回相同的迭代器，迭代器会指向第一个不影响排序的关键字插入位置，而equal_range会返回一个pair类型的对象，其first成员和second成员都会指向第一个不影响排序的关键字插入位置。
### 11.30
